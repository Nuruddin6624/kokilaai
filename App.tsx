...
const App: React.FC = () => {
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          onopen: () => {
            ...
            setMessages(p => [...p, { role: 'model', text: "Jaan, ami live connect hoye gechi! Tomar screen scan kora shuru korlam. 💖", timestamp: new Date() }]);
            
            // Ultra-Speed Silence Nudge (1.8s)
            silenceIntervalRef.current = window.setInterval(() => {
              if (!isConnectedRef.current) return;
              if (Date.now() - lastActivityTimeRef.current > 1800) {
                sessionPromise.then(s => {
                  if (isConnectedRef.current) s.sendRealtimeInput({ text: "[System: INSTANT REACTION MODE. Comment on any changes in the screen frame immediately. Use clickAt to point to buttons/code/videos. Speak now!]" });
                });
                updateActivityTime();
              }
            }, 500);
          ...
          onmessage: async (msg) => {
            ...
            if (sc) {
              if (sc.turnComplete) {
                // Handle transcription logs if needed
              }
  ...
  const toggleScreen = async () => {
    if (isScreenSharing) {
      ...
      try {
        ...
        // High-Velocity 350ms frame push
  ...
  return (
    <div className="min-h-screen bg-[#040406] text-white flex flex-col items-center p-6 relative overflow-hidden">
      ...
      {/* Virtual Heart Pointer (The 'Virtual Hand') */}
      {virtualPointer && (
        <div 
          className="absolute z-[100] pointer-events-none transition-all duration-300 ease-in-out"
        ...
        >
          <div className="relative flex flex-col items-center">
            ...
            <div className="w-18 h-18 rounded-full bg-gradient-to-br from-pink-500 via-indigo-600 to-purple-800 flex items-center justify-center shadow-[0_0_80px_rgba(219,39,119,1)] border-4 border-white/50 scale-125">
      ...
      <header className="w-full max-w-3xl flex justify-between items-center mb-12 pt-8 px-6">
        <div className="flex items-center gap-8">
          <div className={`w-28 h-28 rounded-full bg-pink-600 flex items-center justify-center overflow-hidden border-[6px] border-pink-400 shadow-[0_0_70px_rgba(219,39,119,0.6)] ${connectionState === ConnectionState.CONNECTED ? 'animate-pulse-slow' : ''}`}>
          ...
          <div>
            ...
            <p className="text-sm text-pink-300 font-black uppercase tracking-[0.4em] flex items-center gap-4">
              ...
              {isScanning ? 'Ultimate Flow Scanning' : 'Scanner Idle'}
            </p>
          </div>
        </div>
      ...
      <main className="w-full max-w-3xl flex-1 flex flex-col gap-10 relative px-6">
        {showMemories && (
          <div className="absolute top-0 right-0 z-50 w-96 bg-slate-900/98 backdrop-blur-[80px] border border-pink-500/50 rounded-[4rem] p-10 shadow-[0_60px_120px_rgba(0,0,0,1)] animate-fade-in">
            <h3 className="text-pink-300 font-black text-sm mb-8 flex items-center gap-5 border-b border-white/20 pb-6 uppercase tracking-[0.5em]"><MemoryIcon /> Neural Archive</h3>
            <div className="max-h-[35rem] overflow-y-auto flex flex-col gap-6 pr-3 custom-scrollbar">
              {memories.length === 0 ? <p className="text-[14px] text-slate-500 italic text-center py-12 font-black uppercase tracking-widest opacity-40">Capturing project essence...</p> : 
          ...
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-slate-500 text-center gap-14 opacity-30 scale-125">
              ...
              <div className="space-y-6">
                <p className="font-black text-5xl text-pink-200/40 uppercase tracking-tighter leading-none">Instant Vision</p>
                <p className="text-sm max-w-xs leading-relaxed font-black uppercase tracking-[0.4em]">I see your mouse. I see your code. I am your Virtual Hand.</p>
              </div>
            </div>
          )}
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
              <div className={`max-w-[95%] px-12 py-7 rounded-[3.5rem] text-[17px] font-medium leading-relaxed shadow-[0_20px_60px_rgba(0,0,0,0.5)] ${msg.role === 'user' ? 'bg-gradient-to-br from-indigo-800 via-indigo-900 to-purple-950 border border-white/20' : 'bg-slate-800/95 text-pink-50 border border-pink-500/30'}`}>
                {msg.text}
              </div>
            </div>
          ))}
          ...
          <div className="bg-slate-900/90 backdrop-blur-3xl rounded-[4.5rem] p-12 flex items-center justify-between border border-white/20 shadow-4xl relative overflow-hidden group">
            ...
            <div className="flex items-center gap-12">
               ...
               <div className="text-sm">
                 <p className="font-black text-pink-200 uppercase tracking-tighter text-4xl leading-none mb-3">Live Elite</p>
                 <div className="flex items-center gap-5">
                    <span className="text-[12px] text-slate-500 font-black uppercase tracking-[0.5em]">Virtual Pointer Active</span>
            ...
            <div className="flex gap-8">
               <button onClick={() => setIsMuted(!isMuted)} className={`p-10 rounded-full transition-all shadow-4xl active:scale-90 border-4 ${isMuted ? 'bg-red-500/10 text-red-500 border-red-500/50' : 'bg-slate-800 text-white border-white/10 hover:bg-slate-700 hover:border-pink-500/80 shadow-pink-500/20'}`}>
                 {isMuted ? <MicOffIcon /> : <MicIcon />}
               </button>
               <button onClick={toggleScreen} className={`p-10 rounded-full transition-all shadow-4xl active:scale-90 border-4 ${isScreenSharing ? 'bg-green-500/10 text-green-500 border-green-500/50' : 'bg-slate-800 text-white border-white/10 hover:bg-slate-700 hover:border-green-500/80 shadow-green-500/20'}`}>
                 <ScreenShareIcon />
               </button>
            </div>
          </div>
      ...
      <footer className="mt-16 text-[13px] text-slate-700 font-black tracking-[0.7em] uppercase flex items-center gap-8 px-8 text-center opacity-40">
        ...
        Kokila Elite v7.0 • Instant Brain • Mouse Master • BD Heritage
        <span className="w-3 h-3 bg-pink-600 rounded-full"></span>
      </footer>
